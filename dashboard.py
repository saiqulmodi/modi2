...
# not re-paying the full fetch cost on every rerun. Matters more now that
# both symbols render automatically on every page load rather than only on
# a button click.
_CHAIN_CACHE_TTL_SECONDS = 180
_chain_cache = {}


def _fetch_chain(symbol, strike_range):
    """Fetches, enriches with LTP+volume, and returns the built table for one
    symbol -- the expensive path, only hit on a cache miss."""
    spot_price = get_spot_price(symbol)
    if spot_price is None:
        return None, f"Couldn't fetch a live spot price for {symbol} right now (both Motilal and Angel failed) -- try again in a moment."

    chain_df = get_option_chain(symbol, spot_price, strike_range=strike_range, current_month_only=True)
    if chain_df is None or chain_df.empty:
        return None, f"No option contracts found for {symbol}."

    st.success(f"Fetched {len(chain_df)} contracts fresh for {symbol} (spot ±{strike_range}, all current-month expiries).")

    # current_month_only=True can pull 200+ contracts (all weeklies + the
    # monthly) -- fetching LTP/volume one at a time sequentially would take
    # minutes. Concurrent fetch instead, same pattern as MODI7's
    ...
    # A burst of ~200+ concurrent requests hits Motilal's rate limit and a
    # handful fail outright (verified: retried slowly one at a time, they
    # succeed every time) -- one sequential retry pass, spaced out, mops up
    # the stragglers instead of showing them as permanently N/A.
    ...
    # Multiple expiries are now mixed together (current_month_only=True pulls
    # every weekly + the monthly) -- group by expiry first so same-strike
    # contracts from different expiries don't interleave.
...
def render_symbol(symbol):
    ...
    force_refresh = header_cols[1].button("🔄 Refresh", key=f"refresh_{symbol}")

    cached = None if force_refresh else _chain_cache.get(symbol)
    cache_age = time.time() - cached[0] if cached else None

    try:
        if cached and cache_age < _CHAIN_CACHE_TTL_SECONDS:
            simple_df = cached[1]
            header_cols[0].success(f"Loaded {len(simple_df)} contracts from cache ({cache_age:.0f}s old, spot ±{strike_range}, all current-month expiries).")
        else:
            with st.spinner(f"Fetching live data for {symbol}..."):
                simple_df, error = _fetch_chain(symbol, strike_range)
            if error:
                st.error(error)
                return
    ...
    # concentrated (informal resistance -- a lot of call writing/buying
    # tends to cluster near a level the market expects to cap upside).
    # Highest-volume PUT strike = informal support, same logic on the
    # downside. This is informational only -- it never generates a buy/sell
    # signal on its own, and MODI2 never sells to open a fresh position
    # either way (see live_alerts.py: SELL alerts only ever close an
    # existing tracked BUY, never a new short).
...
# Both symbols render automatically, stacked, on every page load -- no
# typing a symbol or clicking a button just to see the other index. The
# in-memory cache above keeps repeat views fast; the per-symbol Refresh
# button forces a fresh fetch if you want live data before the cache
# expires.
for sym in SYMBOLS:
    render_symbol(sym)
    st.divider()
